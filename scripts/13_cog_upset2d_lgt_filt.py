...
try:
    ...
    out_file = "plots/upset_cog2d_lgt_filt.png"


def make_upset_data(og_count, og_ann_lgt):
    # Filter genes by terpo lgt list
    og_ann_lgt = og_ann_lgt[og_ann_lgt["LGT"] == "trepo"]
    og_ann_lgt = og_ann_lgt.dropna(subset="COG")
    og_ann_lgt["COG"] = og_ann_lgt["COG"].str.split(",")
    og_ann_lgt = og_ann_lgt.explode("COG").reset_index(drop=True)
    # remove unknown func. COG cat
    og_ann_lgt = og_ann_lgt[(og_ann_lgt["COG"] != "S") ]
    og_ann_lgt = og_ann_lgt[(og_ann_lgt["COG"] != "R") ]
    ...
    df_cog =  og_ann_lgt.groupby(["OG", "COG"]).agg(count=('COG', 'size')).\
        reset_index().sort_values(by=["OG", "count"], ascending=[True, False])

    #df_cog = df_cog[(df_cog["COG"] != "S") | (df_cog["COG"] != "R") ]
    df = pd.merge(df_cog, og_count)
    #df = df.drop_duplicates(subset="OG", keep="first") #there is only one duplicated case : OG0004242
...
def upset_plot(file_out: str, df: pd.DataFrame) -> None:
    ...
    #pal= sns.color_palette(cc.cubehelix, n_colors=15)
    #pal = cog_colors()
    ...
    plt.suptitle("Trepo_lgt COG categories og>3")
...
df_upset, df_cog, df = make_upset_data(og_count, og_ann_lgt)

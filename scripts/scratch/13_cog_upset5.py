...
df = df[df["LGT"] == "trepo"] #remove singletons
df = df.dropna(subset=['COG'])
df["COG"] = df["COG"].str.split(",")
df = df.explode("COG").reset_index(drop=True)


g = df.groupby(["OG", "COG"]).size().reset_index().rename(columns={0:"COG_count"})
df_new = pd.merge(df,g).drop_duplicates(subset=["OG", "COG"])
...
df_upset = make_upset_data(df_new)
